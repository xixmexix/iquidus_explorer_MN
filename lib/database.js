...
function update_address_ripple(hash, ledgerHash, txid, amount, type, cb) {
  // Check if address exists
  amount = new BigInteger(amount);
  lib.get_account_balance(hash, ledgerHash, function(balance) {
    find_address(hash, function (address) {
      if (address) {
        // ensure tx doesnt already exist in address.txs
        lib.is_unique(address.txs, txid, function (unique, index) {
          var tx_array = address.txs;
          var received = new BigInteger(address.received);
          var sent = new BigInteger(address.sent);
          if (type === 'vin') {
...
module.exports = {
  ...
  get_block_count: function(since, cb) {
    Tx.distinct(
      'blockindex',
      { "timestamp": { $gt: since } }
  ...
  update_tx_db: function(coin, start, end, timeout, cb) {
    if (settings.baseType === 'ripple') {
        var current = start;
        // current = 184; end = 184; // for testing
        async.during(
            function (cb) {
              return cb(null, current < end);
            },
            function (cb) {
                console.log('Handling index: ' + current);

                lib.get_blockhash(current, function(ledgerHash){
                  if (!ledgerHash) {
                    // Failed to get ledger hash
                    return cb('ERROR: Failed to get ledger hash for index: '+current);
                  }
                  console.log('  hash: ' + ledgerHash);

                  lib.get_block(ledgerHash, function(ledger){
                    if (!ledger.transactions) {
                      console.log('    No transactions.');
                      return cb();
                    }

                    async.eachOf(ledger.transactions, function(tx, idx, eachCb) {
                      console.log('    TX: '+tx.hash);
                      if (tx.TransactionType !== 'Payment') {
                        // Transaction is not a payment, skip it.
                        return eachCb();
                      }

                      if (!tx.metaData || !tx.metaData.TransactionResult || tx.metaData.TransactionResult !== 'tesSUCCESS') {
                        // Transaction not success, skip it.
                        return eachCb();
                      }

                      Tx.findOne({txid: ledgerHash}, function (err, dbTx) {
                        if (dbTx) {
                          // Transaction already exists, continue
                          return eachCb();
                        }

                        // Generate a new transaction for our db.
                        dbTx = new Tx({
                          txid: tx.hash,
                          vin: [
                            {
                          ...
                          timestamp: settings.rippleEpoch + ledger.close_time,
                        ...
                        dbTx.save(function(err) {
                          if (err) {
                            ...
                            update_address_ripple(tx.Account, ledgerHash, tx.hash, +tx.Amount + +tx.Fee, 'vin', function() {
                              // Update received account
                              update_address_ripple(tx.Destination, ledgerHash, tx.hash, tx.Amount, 'vout', function() {
                                return eachCb();
                              })
                            });
                          }
                      ...
                      console.log('  Finished handling transactions.');
